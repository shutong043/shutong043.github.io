---
permalink: /
title: "About"
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

I am a Ph.D. candidate in Computer Science and Technology at the University of Technology Sydney, advised by Prof. Guodong Long. My research focuses on collaborative learning and federated learning. Recently, I have also become interested in agent RL and related topics, and I am actively looking for research collaborations.

I am currently exploring opportunities as an LLM Engineer or Algorithm Engineer, as well as postdoctoral positions and research collaborations. I enjoy work that sits close to both research and systems.

Research Interests
======

* Large language model fine-tuning, LoRA/QLoRA, PEFT, and model merging
* Personalized and federated learning under heterogeneous data distributions
* RAG, parametric memory, tool-use/function calling, and agent workflows
* LLM evaluation, ablation studies, and failure analysis

Selected Publications
======

* **Personalized Additive Modeling for Multi-level Federated Learning**. ICML 2026.
* **FedMerge: Federated Model Merging for Personalization**. AAAI 2026.
* **Spatial Pooling Graph Convolutional Network for Hyperspectral Image Classification**. IEEE TGRS.

Please see the [Publications](/publications/) page for links and details.
